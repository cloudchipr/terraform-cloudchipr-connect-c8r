...
data "aws_iam_policy_document" "CloudchiprStack_read" {
  ...
  statement {
    actions   = ["sts:AssumeRole"]
    resources = ["arn:aws:iam::*:role/${tostring(split(",", var.data)[16])}", "arn:aws:iam::*:role/CloudchiprAccountReadAccessRole", "arn:aws:iam::*:role/CloudchiprAccountReadWriteAccessRole"]
    effect    = "Allow"
  }
